...
namespace Cards_processor{
	...
	uint8_t Deck::GetCard(){
		uint8_t buff=0;
		for (int i=0; i<=51; i++){
			if(cards[i]!=0){buff = cards[i]; cards[i]=0; return buff;}
		}
	...
	string PoketCards(uint8_t card1, uint8_t card2);
	string DostFromCard(uint8_t card);
	...
	string GetCardRank(uint8_t card){
		card &= 0x0F;
		if (card == 13) return "A";
		if (card == 12) return "K";
		if (card == 11) return "Q";
	...
	if ( (card1==0)||(card2==0)||(card3==0)||(card4==0)||(card5==0)||(card6==0)||(card7==0) ) return 0;
...
string PoketCards(uint8_t card1, uint8_t card2){
	...
	//Определяем карманные карты с точки зрения префлопа
	//На первое место ставим большую карту
	if ( (card1&0x0F)>=(card2&0x0F) ){
		pokets = DostFromCard(card1) + DostFromCard(card2);;
	} else {
		pokets = DostFromCard(card2) + DostFromCard(card1);;
	}

	//Если одномастные - добавляем букву s (suited)
	if ((card1&0xF0) == (card2&0xF0)){
		pokets += "s";
	}
	return pokets;
}
